...
create trigger trg_scope_update after update on scope 
 ...
 create trigger trg_ctagentnametype_update after update on ctagentnametype 
   for each row 
    begin 
      insert into auditlogvarchar(action,timestamptouched,dbusername,username,agent_id,for_table,primary_key_value) values ('update',now(),user(),getAgentName(NEW.modified_by_agent_id),NEW.modified_by_agent_id,'ctagentnametype',NEW.type);
    end |
 create trigger trg_ctjournalidentifiertype_update after update on  ctjournalidentifiertype 
   for each row 
    begin 
 ...
 create trigger trg_ctpublicationtype_update after update on  ctpublicationtype 
   for each row 
    begin 
      insert into auditlogvarchar(action,timestamptouched,dbusername,username,agent_id,for_table,primary_key_value) values ('update',now(),user(),getAgentName(NEW.modified_by_agent_id),NEW.modified_by_agent_id,'ctpublicationtype',NEW.publication_type);
    end |
...
create trigger trg_scope_insert after insert on scope 
 ...
 create trigger trg_ctagentnametype_insert after insert on ctagentnametype 
   for each row 
    begin 
      insert into auditlogvarchar(action,timestamptouched,dbusername,username,agent_id,for_table,primary_key_value) values ('insert',now(),user(),getAgentName(NEW.modified_by_agent_id),NEW.modified_by_agent_id,'ctagentnametype',NEW.type);
    end |
 create trigger trg_ctjournalidentifiertype_insert after insert on  ctjournalidentifiertype 
   for each row 
    begin 
      insert into auditlogvarchar(action,timestamptouched,dbusername,username,agent_id,for_table,primary_key_value) values ('insert',now(),user(),getAgentName(NEW.modified_by_agent_id),NEW.modified_by_agent_id,'ctjournalidentifiertype',NEW.identifier_type);
    end |
 create trigger trg_ctpublicationtype_insert after insert on  ctpublicationtype 
   for each row 
    begin 
